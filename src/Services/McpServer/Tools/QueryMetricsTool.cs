...
{
    [McpServerTool(Name = "query_metrics"), Description("Executes a PromQL query against Prometheus to retrieve live metrics such as request rate, error rate, latency, CPU/memory, or target availability. Examples: 'up' to check target health, 'sum(rate(http_server_request_duration_seconds_count{service=\"catalog-api\"}[5m]))' for request rate.")]
